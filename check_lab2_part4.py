...
def killall():
    sh("killall zookld zookd zookfs >/dev/null 2>&1", exit_onerr=False)
...
def check_profileapi_uid():
    ...
    prev_uids = []
    for i in range(60):
        # The -f is needed for pgrep because the process name is python2,
        # not the script name. However, pgrep -f picks up the shell
        # process running the 'for', and one additional process (maybe
        # the one running the entire command?). This is worked around by
        # diagonalization: make the pgrep regex not match itself by
        # using a '\s' to match a space instead of a literal ' '.
        sh("for p in $(pgrep -f python2\\\\s.*profile-s); do stat -c %u /proc/$p 2>/dev/null; done | sort > /tmp/ex3.uids")
        profile_uids = file_read('/tmp/ex3.uids').strip().split('\n')
        # require stability across 2 checks to avoid races
        # assuming that service forks three times
        if profile_uids == prev_uids and len(profile_uids) >= 4:
            break;
        prev_uids = profile_uids
        time.sleep(0.5)
